...
import pic from "../../../assets/game-logos/sf5/portrait-menat-stats-min.jpg"
// Application Screen || Define Exports
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
export default function Menat() {
  return (
    <Accordion className="shine-effect-button">
      <AccordionSummary
      ...
      >
        <Grid container sx={{ display: "flex", justifyContent: "space-evenly" }}>
          ...
          <Grid item xs={10} sx={{ display: "flex", justifyContent: "left", alignItems: "flex-end" }}>
            <Typography variant="h2" mb={2} sx={{ color: "white", textShadow: "2px 2px black" }}>MENAT</Typography>
          </Grid>
        </Grid>
      </AccordionSummary>
      <NormalThrows />
